...
- SQL 与 Python 构建端到端业务指标体系。
- 处理真实 CSV 数据格式问题，包括分号分隔、逗号小数和 0/1 标签。
- 使用 first observed cohort retention 与 RFM 完成客户分层。
- 将促销敏感度从 RFM 分层中拆出，形成更可解释的运营策略矩阵。
- 使用时间切分方式构造未来 180 天不活跃标签，避免全周期建模时间泄漏。
- 实现 Synthetic Control、placebo test 和 injected lift simulation，展示促销策略准 A/B 反事实评估能力。
- 清楚区分相关性分析与因果推断边界。
